...
See [Bitcore-wallet-client-alarmx](https://github.com/alarmxcore/bitcore-wallet-client-alarmx) for the *official* client library that communicates to BWS and verifies its response. Also check [Bitcore-wallet](https://github.com/alarmxcore/bitcore-wallet) for a simple CLI wallet implementation that relies on BWS.
...
```
 ...
 cd bitcore-wallet-service-alarmx && npm start
```
...
BWS supports SSL and Clustering. For a detailed guide on installing BWS with extra features see [Installing BWS](https://github.com/alarmxcore/bitcore-wallet-service-alarmx/blob/master/installation.md).
...
See [Bitcore Wallet Client](https://github.com/alarmxcore/bitcore-wallet-client-alarmx/blob/master/lib/api.js#L73) for implementation details.
...
Returns:
 * Wallet object. (see [fields on the source code](https://github.com/alarmxcore/bitcore-wallet-service-alarmx/blob/master/lib/model/wallet.js)).
...
Returns:
 * List of pending TX Proposals. (see [fields on the source code](https://github.com/alarmxcore/bitcore-wallet-service-alarmx/blob/master/lib/model/txproposal.js))

`/v1/addresses/`: Get Wallet's main addresses (does not include change addresses)

Returns:
 * List of Addresses object: (https://github.com/alarmxcore/bitcore-wallet-service-alarmx/blob/master/lib/model/address.js)).  This call is mainly provided so the client check this addresses for incoming transactions (using a service like [Insight](https://insight.is)
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/alarmxcore/bitcore-wallet-service-alarmx/blob/master/lib/model/txproposal.js)). `.id` is probably needed in this case.


`/v3/addresses/`: Request a new main address from wallet . (creates an address on normal conditions)

Returns:
 * Address object: (https://github.com/alarmxcore/bitcore-wallet-service-alarmx/blob/master/lib/model/address.js)). Note that `path` is returned so client can derive the address independently and check server's response.
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/alarmxcore/bitcore-wallet-service-alarmx/blob/master/lib/model/txproposal.js)). `.status` is probably needed in this case.
